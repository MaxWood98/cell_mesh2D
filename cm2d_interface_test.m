...
cm2dop.meshfrmat = 'flow2d';  %Mesh output format (flow2d / su2_cutcell / su2_dual)
...
cm2dop.nrflood_i = 10;        %Refinement adjacency flooding iterations at the first refinement
cm2dop.nrflood_f = 5;         %Refinement adjacency flooding iterations at the final refinement
cm2dop.nrflood_b = 5;         %Refinement adjacency flooding iterations on boosted refinement
cm2dop.fbound = 3.9;          %Far field distance from object centre  
cm2dop.coffset = [0.0 0.0];   %Object/mesh centre offset (x / y)

%Custom domain bound specification 
cm2dop.set_mbounds = 1;       %Toggle forcing of custom mesh domain bounds
...
%Boundary condition options 
cm2dop.set_custom_bc = 1;     %Set custom boundary conditions in specifed regions (1 = yes | 0 = no)
cm2dop.rem_ffzones = 0;       %Remove any region of the mesh connected to a far field boundary condition
cm2dop.rem_nczones = 1;       %Remove any region of the mesh connected to a non custom set boundary condition
cm2dop.rem_iszones = 1;       %Remove any isolated region of the mesh connected only to a wall boundary condition 
...
% plot(celloftype(:,1),celloftype(:,2),'r.','markersize',20)

% vtxexternal = load('io/vtxexternal');
% plot(vtxexternal(:,1),vtxexternal(:,2),'r.','markersize',20)

% vtx2pert = load('io/vtx2pert');
% plot(vtx2pert(:,1),vtx2pert(:,2),'r.','markersize',20)
...
% plot(6.0000000000000000    ,    3.2812500000000000,'g*')
% plot(5.9999999990962065    ,    3.2812499999333169,'mo')
...
% axis([5.8635    6.0790    3.1598    3.3790]);
% axis([5.8496    6.1364    3.3305    3.6222])
